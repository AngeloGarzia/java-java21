...
public class Stream_03_Test {
	...
	public void test_flatMap() throws Exception {
		...
		List<Pizza> result = orders.stream().flatMap(order -> order.getPizzas().stream())
				.toList();
	...
	public void test_partitionning() throws Exception {
		...
		Map<Boolean, List<Pizza>> result = result = pizzas.stream()
				.collect(Collectors.partitioningBy(pizza ->
						pizza.getName().startsWith("L")));
